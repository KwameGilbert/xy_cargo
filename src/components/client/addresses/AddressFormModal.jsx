...
const AddressFormModal = ({ open, address, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-xs px-2 sm:px-4">
      ...
      />
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-4 sm:p-6 md:p-8 relative flex flex-col"
        style={{ minHeight: 'auto', maxHeight: '90vh', overflowY: 'auto' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg sm:text-xl font-semibold">{form.id ? 'Edit Address' : 'Add New Address'}</h3>
          <button onClick={onClose} className="text-gray-500 text-base sm:text-lg px-2 py-1 rounded-full focus:outline-none">Close</button>
        </div>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex flex-col sm:flex-row justify-end gap-3 mt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 border rounded-md w-full sm:w-auto">Cancel</button>
            <button type="submit" className="px-4 py-2 bg-red-600 text-white rounded-md w-full sm:w-auto">Save Address</button>
          </div>
        </form>
      </div>
    </div>
